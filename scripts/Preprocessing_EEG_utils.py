def epoching(args, data_part, seed):
	...
	for s in range(args.n_ses):
		...
		temp = os.path.join(f"C:\\Users\\gianl\\Desktop\\Materiale vario tesi\\THINGS EEG2\\eeg_dataset\\raw_data\\{'sub-'+format(args.sub,'02')}", 'ses-'+format(s+1,'02'), 'raw_eeg_'+ data_part+'.npy')
		...
		eeg_data = np.load(temp, allow_pickle=True).item()
		...
		epochs = mne.Epochs(raw, events, tmin=0, tmax=0.5, baseline=(0,0.2),
			preload=True)
		del raw
		# Resampling
		if args.sfreq < 1000:
...
def save_prepr(args, whitened_test, whitened_train, img_conditions_train,
	...
	del merged_test
	# Saving directories
	save_dir = os.path.join(args.project_dir, 'preprocessed_data', 'sub-'+format(args.sub,'02'))
