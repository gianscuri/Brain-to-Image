...
class EEGNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.lin_reg = nn.Linear(self.F2 * self.feature_dim, num_classes, bias=False) # va bene per regressione? pare di si input(16*3=48)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        '''
        x = x.unsqueeze(dim=1) # [batch_size, 1, 17, 100]
        x = self.block1(x)
        x = self.block2(x)
